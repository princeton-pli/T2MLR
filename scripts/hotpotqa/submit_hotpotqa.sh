#!/usr/bin/env bash
# Simple submitter for hotpotqa training sweeps (train only, no eval).
...
SFT_RUN_NAME_BASE="${SFT_RUN_NAME_BASE:-hotpotqa_t2mlr}"
LOG_DIR="${LOG_DIR:-$REPO_ROOT/scripts/hotpotqa/slurm}"
TRAIN_SCRIPT="${TRAIN_SCRIPT:-$REPO_ROOT/scripts/hotpotqa/train_hotpotqa.sh}"
SWEEP_PARAMS_YAML="${SWEEP_PARAMS_YAML:-$REPO_ROOT/scripts/hotpotqa/sweep_params.yaml}"
