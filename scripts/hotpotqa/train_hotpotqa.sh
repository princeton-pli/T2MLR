#!/usr/bin/env bash
#SBATCH --job-name=hotpotqa-t2mlr
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --output=scripts/hotpotqa/slurm/hotpotqa-t2mlr-%j.out
#SBATCH --error=scripts/hotpotqa/slurm/hotpotqa-t2mlr-%j.err

set -euo pipefail
export TRANSFORMERS_OFFLINE="${TRANSFORMERS_OFFLINE:-1}"
export HF_HUB_OFFLINE="${HF_HUB_OFFLINE:-1}"
export WANDB_MODE="${WANDB_MODE:-offline}"
export WANDB_RUN_GROUP="${WANDB_RUN_GROUP:-hotpotqa_t2mlr}"
...
DATASET_SLUG="${DATASET_SLUG:-hotpotqa}"
...
LR_SCHEDULER_KW_DEFAULT='{"min_lr_rate":0.02}'
LR_SCHEDULER_KW="${LR_SCHEDULER_KW:-$LR_SCHEDULER_KW_DEFAULT}"
SEED="${SEED:-42}"
...
DEFAULT_NEW_TOKENS="${DEFAULT_NEW_TOKENS:-1024}"
TARGET_LENGTH_BUFFER="${TARGET_LENGTH_BUFFER:-1024}"
...
LABEL_MASK_PROMPT="${LABEL_MASK_PROMPT:-True}"
...
LR_TAG="lr${LEARNING_RATE}"
EPOCHS_TAG="ep${NUM_TRAIN_EPOCHS}"

T2MLR_TAG=$([[ "$T2MLR_ENABLED" == "True" ]] && echo "t2mlr_on" || echo "t2mlr_off")
RUN_NAME_BASE_DEFAULT="${MODEL_SLUG}_${DATASET_SLUG}_${BATCH_FORWARD_TAG}_${T2MLR_TAG}_${WINDOW_TAG}_${PROJECTION_TAG}_${GATE_TAG}_${WEIGHT_TAG}_${FREEZE_TAG}_${LR_TAG}_${EPOCHS_TAG}"
...
PYTHON_COMMAND=(
    ...
    --train_data_path "$TRAIN_DATA_PATH"
    --eval_data_path "$EVAL_DATA_PATH"
    --custom_dataset_preprocessing hotpotqa_cot
    --custom_dataset_postprocessing hotpotqa
...
[[ -n "$MAX_NEW_TOKENS" ]] && PYTHON_COMMAND+=(--max_new_tokens "$MAX_NEW_TOKENS")
[[ "$GATE_BOOL" == "True" ]] && PYTHON_COMMAND+=(--gate_lr_multiplier "$GATE_LR_MULTIPLIER")
...
echo "============================================================================"
echo "[INFO] HotpotQA T2MLR Training"
echo "============================================================================"
